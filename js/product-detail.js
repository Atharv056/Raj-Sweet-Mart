document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateCalculatedPrice() {
        ...
        const selectedUnit = weightUnitSelect ? weightUnitSelect.value : 'kg';
        const grams = selectedUnit === 'kg' ? val * 1000 : val;
        ...
        presetPills.forEach(pill => {
            ...
            if (pillWeight === Math.round(grams)) {
    ...
    if (weightUnitSelect) {
        weightUnitSelect.addEventListener('change', function () {
            ...
            if (newUnit === 'kg') {
                ...
                weightInput.step = '1';
                weightInput.placeholder = 'e.g. 5';
            } else {
                weightInput.value = Math.round(val * 1000);
                weightInput.step = '1';
                weightInput.placeholder = 'e.g. 250';
            }
            updateCalculatedPrice();
        });
    }
    ...
    presetPills.forEach(pill => {
        pill.addEventListener('click', function () {
            const weightInGrams = parseInt(this.getAttribute('data-weight'));
            if (weightUnitSelect) {
                if (weightInGrams >= 1000) {
                    weightUnitSelect.value = 'kg';
                    weightInput.value = weightInGrams / 1000;
                    weightInput.step = '50';
                } else {
    ...
    if (qtyMinusBtn && qtyPlusBtn) {
        qtyMinusBtn.addEventListener('click', function () {
            const selectedUnit = weightUnitSelect ? weightUnitSelect.value : 'kg';
            const step = selectedUnit === 'kg' ? 1 : 50;
            let currentVal = parseFloat(weightInput.value) || 0;
            if (currentVal > 0) {
                weightInput.value = parseFloat(Math.max(0, currentVal - step).toFixed(2));
                updateCalculatedPrice();
            }
        });
        ...
        qtyPlusBtn.addEventListener('click', function () {
            ...
            const step = selectedUnit === 'kg' ? 1 : 50;
            let currentVal = parseFloat(weightInput.value) || 0;
            weightInput.value = parseFloat((currentVal + step).toFixed(2));
            updateCalculatedPrice();
        });
    }
    ...
    document.getElementById('custom-add-btn').addEventListener('click', function (e) {
        ...
        const val = parseFloat(weightInput.value) || 0;
        const selectedUnit = weightUnitSelect ? weightUnitSelect.value : 'kg';
        const grams = selectedUnit === 'kg' ? Math.round(val * 1000) : Math.round(val);
        if (grams <= 0) {
